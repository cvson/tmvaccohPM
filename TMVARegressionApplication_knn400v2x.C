...
void TMVARegressionApplication_knn400v2x( TString myMethodList = "" )
{
    ...
    Float_t muscirange;
    Float_t muironrange, muang_t;
    reader->AddVariable( "muscirange", &muscirange );
    reader->AddVariable( "muironrange", &muironrange );
    reader->AddVariable( "muang_t", &muang_t );
    
    // Spectator variables declared in the training have to be added to the reader, too
    Int_t Ntrack, Ningtrack, inttype, mufc, pene, mupdg;
    Float_t Qsq_true, Qsq_true2, nuE, tmomtrans_true, mumucl, mupe, range;
    ...
    reader->AddSpectator( "Qsq_true2", &Qsq_true2);
    reader->AddSpectator( "nuE", &nuE);
    reader->AddSpectator( "tmomtrans_true", &tmomtrans_true);
    ...
    reader->AddSpectator( "mupe", &mupe);
    reader->AddSpectator( "range", &range);
    
    // --- Book the MVA methods
    
    TString dir    = "weights/";
    TString subname = "_knn400v2xT2";
    ...
    TString fname = "/home/cvson/cc1picoh/frkikawa/meAna/tmva/regression/merged_pmmc_ccqe_bugfix_addtrue_reg.root";

    std::cout << "--- Selecting data sample" << std::endl;
    TFile *pfile = new TFile(fname,"update");
    TTree* theTree = (TTree*)pfile->Get("tree");
    theTree->SetBranchAddress( "muscirange", &muscirange );
    theTree->SetBranchAddress( "muironrange", &muironrange );
    ...
    theTree->SetBranchAddress( "Qsq_true2", &Qsq_true2);
    theTree->SetBranchAddress( "nuE", &nuE);
    theTree->SetBranchAddress( "tmomtrans_true", &tmomtrans_true);
