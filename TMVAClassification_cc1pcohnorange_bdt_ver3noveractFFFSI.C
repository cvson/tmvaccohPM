...
void TMVAClassification_cc1pcohnorange_bdt_ver3noveractFFFSI( TString myMethodList = "" )
{
    //---------------------------------------------------------------
    ...
    TString outfileName( "TMVA_cc1pcohnorange_bdt_ver3noveractFFFSI.root" );//newchange
    TFile* outputFile = TFile::Open( outfileName, "RECREATE" );
    
    // Create the factory object.
    TMVA::Factory *factory = new TMVA::Factory( "TMVAClassification_cc1pcohnorange_ver3noveractFFFSI", outputFile,//newchange
    ...
    TCut preselecet = "Ntrack==2 && pang<90 && !(mufc==0 && pene<9)";
    //FSI definition
    TCut mycuts = preselecet && "abs(inttype)==16 && abs(mupdg)==13 && abs(ppdg)==211 && fileIndex==1"; // for example: TCut mycuts = "abs(var1)<0.5 && abs(var2-0.5)<1";
    TCut mycutb = preselecet && "(!(abs(inttype)==16 && abs(mupdg)==13 && abs(ppdg)==211 && fileIndex==1) || fileIndex>1)"; // for example: TCut mycutb = "abs(var1)<0.5";
