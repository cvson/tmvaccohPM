...
Int_t findbinValy(TH1* h1, Double_t yval);
   ...
   while ( (info = (MethodInfo*)next()) ) {
      ...
      if (info->methodTitle.Contains("Cuts")) {
         ...
         //info->effpurS->GetXaxis()->SetTitle( TString("Cut value applied on ") + info->methodTitle + " output" );
          info->effpurS->GetXaxis()->SetTitle( "Cut applied on discriminant parameter" );
          info->effpurS->GetXaxis()->CenterTitle();
      }
      info->effpurS->GetYaxis()->SetTitle( "Efficiency (Purity)" );
       info->effpurS->GetYaxis()->CenterTitle();
      ...
      info->purS->Draw("samehistl");
       Float_t meCut = info->purS->GetBinCenter(findbinValy(info->purS,0.533));
       cout<<"Same purity at bin "<<findbinValy(info->purS,0.533)<<" cut "<<meCut<<endl;
       cout<<"Corresponding to efficiency "<<info->sigE->GetBinContent(findbinValy(info->purS,0.533))<<endl;

       TLine *pPurLine = new TLine(meCut,0,meCut,1);
       pPurLine->SetLineColor(kRed);
       pPurLine->SetLineWidth(2);
       //pPurLine->Draw("same");
      // overlay signal and background histograms
      info->sigE->Draw("samehistl");
      info->bgdE->Draw("samehistl");

       /*TLine *pOffEfficiency = new TLine(0,0.323,1,0.323);
       pOffEfficiency->SetLineWidth(3);
       pOffEfficiency->SetLineColor(kRed);*/
      ...
      //info->line1 = tl.DrawLatex( 0.15, 0.23, Form("For %1.0f signal and %1.0f background", fNSignal, fNBackground));
       info->line1 = tl.DrawLatex( 0.15, 0.75, Form("For %1.0f signal and %1.0f background", fNSignal, fNBackground));
     // tl.DrawLatex( 0.15, 0.19, "events the maximum "+GetLatexFormula()+" is");
      ...
      if (Save_Images) {
         TMVAGlob::imgconv( c, Form("plots/mvaeffs_nopionAngle_%s", info->methodTitle.Data()) ); 
      }
      countCanvas++;
   }
}
...
Int_t findbinValy(TH1* h1, Double_t yval){
    ...
    cout<<"Number of xbins "<<nbinx<<endl;
    //Int_t nbiny = h1->GetNbinsY();
    if (yval<=h1->GetBinContent(1)) return 1;
    else if (yval>=h1->GetBinContent(nbinx)) return nbinx;
    for (Int_t ibx=1; ibx<=nbinx; ibx++) {
        Float_t bincont = h1->GetBinContent(ibx);
        Float_t bincontNext = h1->GetBinContent(ibx+1);
        if (yval>bincont && yval<bincontNext) {
            foundxbin = ibx;
        }
        /*for (Int_t iby==1; iby<=nbiny; iby++) {
            Float_t bincont = h1->GetBinContent(ibx,iby);
        }*/
    }
              ...
              Bool_t useTMVAStyle = kTRUE, TString formula="S/sqrt(S+B)" )
{
   TMVAGlob::Initialize( useTMVAStyle );
    //Signal veract entries 34257 weight 34257
    //Background veract entries 733335 weight 733791
    //Int_t realnbkgNorm = int(733791*1000/float(34257));//this is for full sample
    Int_t realnbkgNorm = int(131042*1000/float(6290));
   //StatDialogMVAEffs* gGui = new StatDialogMVAEffs(gClient->GetRoot(), 1000, 1000);
    StatDialogMVAEffs* gGui = new StatDialogMVAEffs(gClient->GetRoot(), 1000, realnbkgNorm);
