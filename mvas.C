   ...
   while ((key = (TKey*)next())) {
	 ...
	 std::cout << "--- Found directory for method: " << methodName << "::" << methodTitle << std::flush;
         ...
         //frame->GetXaxis()->SetTitle( methodTitle + ((htype == MVAType || htype == CompareType) ? " response" : "") );
          frame->GetXaxis()->SetTitle(" Discriminant parameter ");
         ...
         if (Save_Images) {
             if      (htype == MVAType)    TMVAGlob::imgconv( c, Form("plots/mva_%s",     methodTitle.Data()) );
                ...
                TMVAGlob::imgconv( c, Form("plots/overtrain_ver3_%s", methodTitle.Data()) );
                c->SetLogy();
                c->Update();
                TMVAGlob::imgconv( c, Form("plots/overtrain_logy_ver3_%s", methodTitle.Data()) );
