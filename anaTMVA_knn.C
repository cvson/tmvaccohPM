...
void anaTMVA_knn::Loop(Bool_t isTrainTree, TString ipFile)
{
    TString subname = "anaTMVA_knn";
    ...
    for (Long64_t jentry=0; jentry<nentries;jentry++) {
        ...
        if (classID==0) hnuE_all_sig->Fill(nuE,totweight);
        if (classID==1) hnuE_all_bkg->Fill(nuE,totweight);
        if (CutMVA(ientry,knnCut)>0) {
