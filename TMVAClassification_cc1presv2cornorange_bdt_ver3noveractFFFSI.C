...
void TMVAClassification_cc1presv2cornorange_bdt_ver3noveractFFFSI( TString myMethodList = "" )
{
    //---------------------------------------------------------------
    ...
    TString outfileName( "TMVA_cc1presv2cornorange_bdt_ver3noveractFFFSI.root" );//newchange
    TFile* outputFile = TFile::Open( outfileName, "RECREATE" );
    
    // Create the factory object.
    TMVA::Factory *factory = new TMVA::Factory( "TMVAClassification_cc1presv2cornorange_ver3noveractFFFSI", outputFile,//newchange
    ...
    // ---------------------------------------------------------------
    // --- Get weight
    TString fratioStr="/home/cvson/scraid2/cc1picoh/FIT/data/nd34_tuned_11bv3.1_250ka.root";
    
    
    
    // ---------------------------------------------------------------
    // --- Add sample
    //this is with prange/pang_t fixed
    ...
    TCut preselecet = "Ntrack==2 && pang<90 && !(mufc==0 && pene<9) && !(abs(inttype)==16)";
    
    TCut mycuts = preselecet && "(abs(inttype)==11 || abs(inttype)==12  || abs(inttype)==13) && fileIndex==1 && abs(mupdg)==13 && abs(ppdg)==211"; // for example: TCut mycuts = "abs(var1)<0.5 && abs(var2-0.5)<1";
    TCut mycutb = preselecet && "!((abs(inttype)==11 || abs(inttype)==12  || abs(inttype)==13) && fileIndex==1 && abs(mupdg)==13 && abs(ppdg)==211)"; // for example: TCut mycutb = "abs(var1)<0.5";
